	...
	SubShader
	{
		...
		Pass
		{
			...
			half4 frag (v2f i) : SV_Target
			{
				...
				ShadowThreshold = saturate((ShadowThreshold - _BottomThreshold) * scale);

				float2 p11_22 = float2(unity_CameraProjection._11, unity_CameraProjection._22);
		        float3 vpos = float3((i.uv * 2 - 1) / p11_22, -1) * depth;
		        float4 wpos = mul(_CamToWorld, float4(vpos, 0));
		        wpos += float4(_WorldSpaceCameraPos, 0) / _ProjectionParams.z - (_SunDirection*0.04);

				half4 ShadowColor = tex2D(_CloudTex, -wpos.xz * _CloudTexScale * _ProjectionParams.z + offset);
				float up = dot(float3(0, 1, 0), normal);
				up = step(ShadowThreshold, up);
				float4 source = tex2D(_MainTex, i.uv);
				float4 col = lerp(source, source / ShadowColor, -up * _CloudShadowIntensity * ShadowThreshold);
				return col;
			}
			ENDCG
		}
