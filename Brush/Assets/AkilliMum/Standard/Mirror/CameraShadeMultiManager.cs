...
                    //shadows?
                    if (_originalCameraShades[camIndex].Shadow == false && ShadowDepth > depth)
                    {
                        copy.Shadow = false;
                    }
        ...
        public void RenderReflective()
        {
            if (_originalCameraShades.Count < 3)
            {
                ...
                for (int eachCam = 0; eachCam < _originalCameraShades.Count; eachCam++)
                {
                    ...
                    if (!_originalCameraShades[eachCam].IsObjectVisible(_attachedCam))
                    {
#if DEBUG_LOG
                ...
                .RenderReflective(cameraList[eachDepth - 1], true, false)[0];
#if DEBUG_LOG
                    ...
                    for (int eachDepth = Depth - 1; eachDepth >= 0; eachDepth--)
                    {
                        ...
                        _copyCameraShades[nextCamIndex, eachDepth]
                          .RenderReflective(cameraList[eachDepth], nextCamIndex == eachCam);
                ...
                for (int eachCam = 0; eachCam < _originalCameraShades.Count; eachCam++)
                {
                    ...
                    _copyCameraShades[eachCam, 0].UpdateMaterialProperties(null);
                }
            }
            else
            {
                ...
                for (int eachCam = 0; eachCam < _originalCameraShades.Count; eachCam++)
                {
                    ...
                    cameraList[order] = _originalCameraShades[eachCam].RenderReflective(null, true, false)[0];


                    //draw fakes recursion
                    for (int disableCam = 0; disableCam < _originalCameraShades.Count; disableCam++)
                    {
                        var camera = _originalCameraShades[disableCam].RenderReflective(cameraList[1], true, false)[0];
                        ...
                        _copyCameraShades[disableCam, 2].RenderReflective(camera, true, true);
                    ...
                    for (int toDrawCam = 0; toDrawCam < _originalCameraShades.Count; toDrawCam++)
                    {
                        ...
                        for (int disableCam = 0; disableCam < _originalCameraShades.Count; disableCam++)
                        {
                            ...
                            _copyCameraShades[disableCam, 2].UpdateMaterialProperties(null);
                        }

                        //draw first recursion
                        _copyCameraShades[nextCam, 1].RenderReflective(cameraList[1], false, true);
                    ...
                    for (int toDrawCam = 0; toDrawCam < _originalCameraShades.Count; toDrawCam++)
                    {
                        ...
                        _copyCameraShades[toDrawCam, 1].UpdateMaterialProperties(null);
                    }

                    //real one
                    _copyCameraShades[eachCam, 0].RenderReflective(null, true, true);
