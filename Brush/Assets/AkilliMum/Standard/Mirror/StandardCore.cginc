...
float4 GetReflectionTexture(float eyeIndex, float2 uv){
    if(eyeIndex == 0.){
        if(_LODLevel < 1.){
            return tex2D(_ReflectionTex, uv);
        }
        else{
            return tex2Dlod (_ReflectionTex, float4(uv,0,_LODLevel));
        }
    }
    else{
        if(_LODLevel < 1.){
            return tex2D(_ReflectionTexOther, uv);
        }
        else{
            return tex2Dlod (_ReflectionTexOther, float4(uv,0,_LODLevel));
        }
    }
    ...
    //        lerp
    //        (
    //            tex2D(_ReflectionTex, uv),
    //            tex2Dlod (_ReflectionTex, float4(uv,0,_LODLevel)),
    //            _LODLevel
    //        ),
    //        lerp
    //        (
    //            tex2D(_ReflectionTexOther, uv),
    //            tex2Dlod (_ReflectionTexOther, float4(uv,0,_LODLevel)),
    //            _LODLevel
    //        ),
    //        eyeIndex
    //    );
...
float GetDepthTexture(float eyeIndex, float4 screenPos){
    if(eyeIndex == 0.){
        return tex2Dproj(_ReflectionTexDepth, UNITY_PROJ_COORD(screenPos)).r;
    }
    else{
        return tex2Dproj(_ReflectionTexOtherDepth, UNITY_PROJ_COORD(screenPos)).r;
    }
    //return
    //    lerp
    //    (
    //        tex2Dproj(_ReflectionTexDepth, UNITY_PROJ_COORD(screenPos)).r,
    //        tex2Dproj(_ReflectionTexOtherDepth, UNITY_PROJ_COORD(screenPos)).r,
    //        eyeIndex
    //    );
}
...
inline float4 CalculateReflection (float4 screenPos, float texdistance, float eyeIndex, 
    /*float4 eyePos,*/inout half smoothness,inout half3 specColor,
    float2 texuv, inout float3 normal, inout half3 diffColor,
    bool isForward, inout half3 emissiveColor, inout half texAlpha)
{
    //!!important eyeIndex is calculated inside related shader code, 
    //unity_stereoEyeIndex gets wrong if we try to get here!!!!
    ...
    #if UNITY_SINGLE_PASS_STEREO //todo: should we move this to frag too?
        // If Single-Pass Stereo mode is active, transform the
        // coordinates to get the correct output UV for the current eye.
        float4 scaleOffset = unity_StereoScaleOffset[eyeIndex];
    ...
    if(_EnableWave > 0)
    {
        ...
        reflection = GetReflectionTexture(eyeIndex, screenUV1) / 2 +
                     GetReflectionTexture(eyeIndex, screenUV2) / 2;
    }
    ...
    if(_EnableDepthBlur>0)
    {
        ...
        texAlpha = _ReflectionIntensity;
        
        //reflection is already sampled so no need to sample it here
        //reflection = GetReflectionTexture(eyeIndex, screenUV);
        //but get the alpha from tex
        //_ReflectionIntensity = reflection.a; //change reflection intensity!!
        //texAlpha = reflection.a; //and alpha
    }
    else if(_EnableSimpleDepth>0)
    {

        float sceneDepthAtFrag = GetDepthTexture(eyeIndex, screenPos);
        ...
        depth = clamp(pow(depth, _SimpleDepthCutoff * texdistance), 0., 1.);
...
half4 fragForwardBaseInternal (VertexOutputForwardBase i)
{
    ...
    half3 emissiveColor;
    float4 reflection = CalculateReflection(i.screenPos, i.distance, i.eyeIndex, /*i.eyePos,*/ s.smoothness, s.specColor,
    ...
    #else
        half3 spec = s.specColor;
        half ref = s.oneMinusReflectivity;
        half smooth = s.smoothness;
        
        if(_MixBlackColor > 0 && _ReflectionIntensity > 0.99){
            //float3 check = float3(0, 0, 0);
            //if (!all(check.rgb == reflection.rgb)){
            if (reflection.r > 0.001 && reflection.g > 0.001 && reflection.b > 0.001){
                spec = 0;
...
)
{
    ...
    data.normalWorld    = s.normalWorld;
        ...
        if(_MixBlackColor > 0 && _ReflectionIntensity > 0.99){
            ...
            //if (!all(check.rgb == reflection.rgb)){
            if (reflection.r > 0.001 && reflection.g > 0.001 && reflection.b > 0.001){
                outGBuffer0 = reflection/3;
                outGBuffer1 = reflection/3;
                outGBuffer2 = reflection/3;
                
                //reflection.rgb = diffColor.rgb;
                // emissiveColor = reflection;
                // data.diffuseColor = reflection;
                //UnityStandardDataToGbuffer(data, outGBuffer0, outGBuffer1, outGBuffer2);
            }
        }
    #endif
    ...
    //outEmission = half4(data.diffuseColor,1)  ; //half4(emissiveColor, 1);
